	...
	function initWebUpload(item, options) {
		...
		uploader.on('error', function(type, param1) {
			if (type == 'Q_TYPE_DENIED') {
				showPrompt('文件上传类型不匹配', 'error');
			} else if (type == 'Q_EXCEED_NUM_LIMIT') {
				showPrompt('文件上传数量超出限制', 'error');
			} else if (type == 'Q_EXCEED_SIZE_LIMIT') {
				showPrompt('文件总大小超出限制', 'error');
			} else if (type == 'F_EXCEED_SIZE') {
				showPrompt(opts.fileSingleSizeLimitMsg || '上传的文件大小不能大于10M', 'error');
			} else if (type == 'F_DUPLICATE') {
				showPrompt('该文件已经上传', 'error');
			} else {
				showPrompt('上传失败，请稍候重试', 'error');
			}
			return false;
		});
	}
